...
let StrategyPoints = {
	...
	RefreshBuyableForgePoints: (formula) => {
		...
		if($('div.buyable-fp').length === 0) {
			$('#hammerBar .fpContainer').append(`<div class="buyable-fp barItem" title="${FH.HTML.Tooltip(FH.t('StrategyPoints.BuyableFP'))}">${ FH.HTML.Format(amount)}</div>`);
	...
	RefreshBar: () => {
		...
		if( $('#hammerBar').length < 1 ){
			...
			}).append( `<div class="fpContainer"><div class="fp-storage barItem">0</div></div>` );
