...
FoEproxy.addHandler('ItemStoreService', 'getConfigs', (data, postData) => {
	...
	if (cleaned) HammerStorage.setItem("shopAssist.favourites",JSON.stringify(shopAssist.favourites));
	...
	if (cleaned) HammerStorage.setItem("shopAssist.alerts",JSON.stringify(shopAssist.alerts));
	HammerStorage.setItem("shopAssist.shopMeta",JSON.stringify(shopAssist.shopMeta));
});
...
let shopAssist = {
	...
	storeId:null,
	alertsTriggered: {},
	shopMeta:JSON.parse(HammerStorage.getItem("shopAssist.shopMeta")||"{}"),
	favourites: JSON.parse(HammerStorage.getItem("shopAssist.favourites")||"{}"),
	favouritesOnly: JSON.parse(HammerStorage.getItem("shopAssist.favouritesOnly")||"false"),
	currencyfilter:{},
	alerts: JSON.parse(HammerStorage.getItem("shopAssist.alerts")||"{}"),
	timeout: null,
    /**
	...
	updateDialog: () => {
		...
		$("#shopAssistFav").on("change",function(e){
			...
			HammerStorage.setItem("shopAssist.favouritesOnly",JSON.stringify(shopAssist.favouritesOnly));
		...
		$("#shopAssistUnlock").on("change",function(e){
			...
			HammerStorage.setItem("shopAssist.unlockedOnly",JSON.stringify(shopAssist.unlockedOnly));
			if (shopAssist.unlockedOnly) {
				$(".shopAssistTable").addClass("unlockedOnly");
			} else {
		...
		$(".shopFavourite").on("click",function(e){
			...
			HammerStorage.setItem("shopAssist.favourites",JSON.stringify(shopAssist.favourites));
		...
		$(".shopAlert").on("click",function(e){
			...
			HammerStorage.setItem("shopAssist.alerts",JSON.stringify(shopAssist.alerts));
    ...
    SaveSettings: () => {
		...
		if ($("#shopAssistAutoOpen").is(':checked'))
			value = true;
		HammerStorage.setItem('ShowShopAssist', value);
